...
func (d *Helper) daemonSetFilter(pod corev1.Pod) podDeleteStatus {
	...
	fmt.Fprintf(d.ErrOut, "pod is part of daemon-set %q\n", pod.Name)
	if _, err := d.Client.AppsV1().DaemonSets(pod.Namespace).Get(controllerRef.Name, metav1.GetOptions{}); err != nil {
		fmt.Fprintf(d.ErrOut, "error getting daemonset for pod %q\n, %v", pod.Name, err)
	...
	if !d.IgnoreAllDaemonSets {
		fmt.Fprintf(d.ErrOut, "not ignore daemonsets for pod %q\n", pod.Name)
		return makePodDeleteStatusWithError(daemonSetFatal)
	}
	fmt.Fprintf(d.ErrOut, "last daemonset filter return for pod %q\n", pod.Name)
