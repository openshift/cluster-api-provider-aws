...
// registerWithClassicLoadBalancers registers instance with clasic load balancer
func (lbr *actuatorRuntime) registerWithClassicLoadBalancers(names []string, instance *ec2.Instance) error {
	...
	for _, elbName := range names {
		...
		_, err := lbr.awsclient.RegisterInstancesWithLoadBalancer(req)
		if err != nil {
...
// registerWithNetworkLoadBalancers registers instance with network load balancer
func (lbr *actuatorRuntime) registerWithNetworkLoadBalancers(names []string, instance *ec2.Instance) error {
	...
	lbsResponse, err := lbr.awsclient.ELBv2DescribeLoadBalancers(lbsRequest)
	if err != nil {
	...
	for _, loadBalancer := range lbsResponse.LoadBalancers {
		...
		targetGroupsOutput, err := lbr.awsclient.ELBv2DescribeTargetGroups(targetGroupsInput)
		if err != nil {
	...
	for _, targetGroup := range targetGroups {
		...
		_, err := lbr.awsclient.ELBv2RegisterTargets(registerTargetsInput)
		if err != nil {
